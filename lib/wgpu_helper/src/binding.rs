...
impl<'l> BindGroupBuilder<'l> {
  ...
  pub fn push_resources(
    mut self,
    resources: Vec<wgpu::BindingResource<'l>>,
  ) -> Self {
  ...
  fn resource(mut self, resource: wgpu::BindingResource<'l>) -> Self {
    // if !(self.entries.len() < self.layout_with_desc.entries.len()) {
    //   panic!(
...
impl<C: BindGroupContent> BindGroup<C> {
  ...
  pub fn new_vis(
    ...
    let bind_group_layout =
      Self::create_bind_group_layout_vis(device, None, visibility);
